...
class Index extends Component {
    ...
    loadEdit({ node, path, treeIndex }){
      ...
      .join(',\n   ');
      //console.log(node);
      $this.props.history.push('/positionmenu/edit/'+node._id);
    }
    ...
    getListMenu(){
      axioApi.get('/api/positionmenu/getAll').then((res) => {
        $this.setState({
          listitems: res.data,
          treeData: res.data
        })
    ...
    deletePost(id){
        axioApi.post('/api/positionmenu/remove', {_id : id}).then((res) => {
            $this.getDats()
        });
    }
    tabRows(){
      return $this.state.posts.map(function(post){
          ...
          <td className="text-center" style={{width:'120px'}}>
              <Link to={"/positionmenu/edit/"+post._id}>
      ...
      <div className="animated fadeIn">
        <Row>
          <Col>
            <Card>
              <CardHeader>
                <i className="fa fa-align-justify"></i> Danh sách vị trí menu
                <Link to="/positionmenu/create"><button className="btn btn-sm btn-success flor"><i className="fa fa-plus" aria-hidden="true"></i> Thêm</button></Link>
              </CardHeader>
